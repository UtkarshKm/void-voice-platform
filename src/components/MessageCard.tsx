...
function MessageCard({message, onMessageDelete}: MessageCardProps) {
  ...
  return (
    <Card className="w-full bg-card border-border shadow-sm hover:shadow-md transition-shadow">
      ...
      {/* ✅ Simplified Footer - Removed ID and "Anonymous feedback received" */}
      <div className="px-6 pb-4">
        <div className="flex items-center justify-center pt-3 border-t border-border/50">
          <span className="flex items-center space-x-2 text-xs text-muted-foreground">
            <span className="w-2 h-2 bg-primary rounded-full animate-pulse"></span>
            <span>Voice from the void</span>
          </span>
        </div>
      </div>
    </Card>
